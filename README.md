...
- OAuthのdevice code、更新トークン、MicrosoftアクセストークンはReactやゲーム／Modプロセスへ渡しません。Minecraftアクセストークンは既定では渡さず、インスタンスのPermissionsで「アクセストークンの受け渡し」をONにした場合だけゲームとすべてのModから利用可能になります。更新トークンはWindows資格情報マネージャーへ保存します。外部応答、アーカイブ、ダウンロード、ローカル設定には件数・サイズ・パス・配布元の検証を行います。
