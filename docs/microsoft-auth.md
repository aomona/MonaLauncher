...
オフライン起動のIPCにはofflineUsernameが必須。UIの入力DialogとRust側の両方で半角英数字・_の1〜16文字を検証する。他モードでの名前指定は拒否する。起動処理の最終段でもオフライン用の名前だけを取り出して検証し、アカウントのUUIDや認証ブローカーを引き継がない。ユーザー名は今回の起動引数へ渡し、インスタンスの保存内容やMicrosoftアカウントは変更しない。
