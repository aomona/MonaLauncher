...
保存中はSaving…、完了時は同じ場所で短くSaved。成功Toastを出さない。失敗時はDraftを残し、原因とRetryを近傍へ表示する。保存が失敗した値を保存済みとして扱わない。切替型Controlの保存失敗は最後の確定値へ戻し、失敗と再試行の導線を表示する。
...
右下、最大3件。通常成功通知を大量に出さない。コピー完了・設定保存は原則操作した場所で伝える。失敗をToastだけに置き、消えたら原因が分からなくなる設計にしない。

Modal内の処理はInlineを優先。Toastが必要なら前面の操作可能な領域内の右下へ表示し、Modal外のinertな領域に操作付きToastを置かない。Hover/Focus中は自動終了を止める。重要エラーは自動消去しない。
