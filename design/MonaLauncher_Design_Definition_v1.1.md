...
Mainの最大幅はHome・Instances・News・Galleryでは上限なし、Settings 960px。いずれも左揃え。Home・Newsも共通のPage paddingを保ち、利用可能な横幅いっぱいに表示する。
...
Menuは「オフラインモードで起動」「デモモードで起動」「複製」「削除」の順。起動モード指定は今回限りとし、保存済みモードやPermissionsを変更しない。オフラインはユーザー名入力Dialogを開き、対象Instance名、初期値Playerの入力欄、Cancelと起動を表示する。半角英数字とアンダースコア1〜16文字を受け付け、空欄・不正入力では起動を無効にする。入力欄に初期Focusを置き、Enterまたは起動ボタンで今回のユーザー名を確定して起動する。IME変換中のEnterでは起動しない。Cancel・Escでは起動せずMenuの起点へFocusを戻す。Microsoft認証・認証仲介を使わず、通信許可は既存Permissionsに従う。複製は別ID・別gameディレクトリへワールド・Mod・設定と管理メタデータをコピーしてOverviewを開く。共有Java・ライブラリ・アセットは再利用し、起動用データ・キャッシュ・profilekeysはコピーしない。複製中は状態を表示し、失敗時は元データを保持してエラーを表示する。削除は対象名の一致入力を必要とする既存確認Dialogへ進む。実行中・処理中は起動・複製・削除を無効にする。MenuはBase UIを使用し、キーボード操作とEscでの閉じる操作・Focus復帰を提供する。
...
記事Modalの最大幅は専用トークンの1000pxとし、周囲32px/狭幅16px、タイトルと閉じる操作を固定して本文だけScrollする。長いタイトルは最大25dvhの領域内でScrollでき、文字拡大で幅が不足した場合は閉じる操作をタイトルの上段へ送る。本文は選択可能。共通DialogでFocusを閉じ込め、Esc/Backdrop/閉じるボタンで元の記事RowへFocusを戻す。Modal表示中は背景のページ切替Shortcutを無効にする。取得済み本文はオフラインでも表示する。配信元ごとにキャッシュを分離し、一部取得失敗でも他の記事を隠さない。
