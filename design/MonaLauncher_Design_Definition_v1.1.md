...
Permissionsでは、このインスタンスのゲーム全体に適用する権限を表示する。ゲームデータへの書き込みとナレーターはSwitchで変更・自動保存し、次回起動から適用する。既存インスタンスは従来の許可状態を引き継ぐ。通信・共有コードの読み取り・画面/入力/通常音声など未対応の変更は操作可能にせず、固定の状態として示す。実行中・保存中・Mod操作中・未対応OSでは編集を禁止し、理由を表示する。保存失敗時は確定値を維持し、近傍の再試行で同じ要求を送る。狭幅ではSwitchを説明文の下へ移す。値・寸法は既存Switchトークンを使う。LinuxではWaylandを優先し、Wayland使用時はX11接続を公開しないこと、X11/XWayland使用時は他アプリ操作も可能であること、PulseAudio接続は録音も含む互換許可であることを明示する。
...
TabsはAll / Minecraft / MonaLauncher。テキスト主体のRow、左に画像がある場合だけ160×90pxのThumbnail。画像がない記事のために装飾用Placeholderを増やさない。

記事名、短い要約、配信元、日時を表示する。記事名は標準2行まで、要約2行まで。本文と画像間16px。幅不足時はThumbnailを本文の上へ移せる。スクリーンショットのHoverメタデータ方式をNewsへ流用しない。

記事は幅最大880px、高さは利用可能Viewport内のArticle Modalへ。本文は16px/28px、行の長さは最大65ch。Header固定、記事本体Scroll。記事タイトル、配信元、公開日時、原文へのモノクロリンクを表示する。

外部記事は閲覧専用。スクリプトやアプリ権限へのアクセスを認めない設計とする。本文を取得・安全に表示できないときは、その理由と原文を開く導線を表示する。欠けた本文を推測して作らない。外部リンクの実行先はOSの既定ブラウザとし、その動作を示す。
