...
MonaLauncherの記事はRow全体から記事Modalを開く。タイトル・配信元・日付・著者（任意）とRSS内の本文を表示する。本文は許可した見出し・段落・リスト・引用・コード・表・HTTPSリンクに限定してサニタイズする。外部画像・埋め込み・フォームは読み込まない。本文リンクは既定ブラウザで開き、失敗時はModal内にエラーを残す。クリック方法の案内文は表示しない。
