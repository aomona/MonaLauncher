...
Menuは「オフラインモードで起動」「デモモードで起動」「複製」「削除」の順。起動モード指定は今回限りとし、保存済みモードやPermissionsを変更しない。オフラインはMicrosoft認証・認証仲介を使わず起動し、通信許可は既存Permissionsに従う。複製は別ID・別gameディレクトリへワールド・Mod・設定と管理メタデータをコピーしてOverviewを開く。共有Java・ライブラリ・アセットは再利用し、起動用データ・キャッシュ・profilekeysはコピーしない。複製中は状態を表示し、失敗時は元データを保持してエラーを表示する。削除は対象名の一致入力を必要とする既存確認Dialogへ進む。実行中・処理中は起動・複製・削除を無効にする。MenuはBase UIを使用し、キーボード操作とEscでの閉じる操作・Focus復帰を提供する。
